...
class Atomic(object):
                ...
                with tarfile.open(self.args.image, 'r') as t:
                    ...
                    if os.path.exists(manifest_file):
                        ...
                        for m in json.loads(manifest):
                            regloc, image, tag = self._parse_imagename(m["RepoTags"][0])
                        ...
                        regloc, image, tag = self._parse_imagename(list(json.loads(repositories).keys())[0])
                        imagebranch = "dockerimg/%s-%s" % (image, tag)
                        input_layers = []
    ...
    def _prune_ostree_images(self):
        ...
        def visit(rev):
            commit = repo.resolve_rev(rev, False)[1]
            manifest = self._get_commit_metadata (repo, commit, "docker.manifest")
            if not manifest:
                return
            for layer in self._get_layers_from_manifest(manifest):
